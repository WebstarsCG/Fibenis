	...
	if(default_data.length!=0){
		...
		for(var default_i = 0; default_i<lv['default_data'].length; default_i++){
			...
			for( var default_j = 0 ; default_j<lv['default_data'][default_i].length; default_j++){
					...
					lv['col_element'] = document.getElementById(`ft_col_${col_i}_${field_id}`);

					if(row_i==1){
						if(lv['col_element'].dataset.ftColData!=undefined){
							lv.col_data[col_i]=JSON.parse(lv['col_element'].dataset.ftColData);
						}
					}
					...
					if(lv['default_data'][default_i][default_j].ms_name){
						...
						if(lv['col_element'].dataset.ftColData==undefined){
							...
							E_V_PASS(row_i+'_ft_'+col_i+'_'+field_id,lv.col_data[col_i][lv['default_data'][default_i][default_j]]);
							document.getElementById(row_i+'_ft_'+col_i+'_'+field_id).dataset.ftValue=lv['default_data'][default_i][default_j];
						}
					}
...
function getfibenisTableDataById(field_id) {
...
function get_fibenis_table_data_engine(field_id){
	...
	for(row_count_i=0;row_count_i<row_count; row_count_i++){
		...
		for(var col_i =0; col_i<colCount; col_i++) {
				...
				else if(temp_cell.dataset.ftType == 'ftlabelvalue'){
					data.push(document.getElementById(eid).dataset.ftValue);
				}													
...
function pre_fill_fiben_grid_form_data(param){
	...
	E_V_PASS(element.id,decodeURI(encodeURI(element.value)));			
	var key_id = param.plugin_key;
													
	if(element.value){
	
		var fibenistableData= JSON.parse(decodeURI(encodeURI(element.value)));
		...
		for(row_i = 0; row_i<fibenistableData.length;  row_i++){
				...
				for(col_idx=0;col_idx<fibenTableColLen;col_idx++){
						...
						if(cell.row_id==1){
							...
							lv.colPriElement = document.getElementById(`ft_col_${cell.col_id}_${key_id}`);							
							if(lv.colPriElement.dataset.ftColData!=undefined){
								lv.colData[cell.col_id]=JSON.parse(lv.colPriElement.dataset.ftColData);
							}
						}
						...
						}else if(cell.col_ele.dataset.ftType=='ftlabelvalue'){								
							E_V_PASS(temp_key,lv.colData[cell.col_id][cell.value]);
							document.getElementById(temp_key).dataset.ftValue=cell.value;							
						}else{
